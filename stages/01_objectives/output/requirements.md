...
Anything not stated above — compliance/disclaimer content, precise financial modeling, feature additions in the name of completeness. None of this is in the brief; none of it belongs in the build.

One documented exception (workspace decision, not from the brief): a full-name field, outside the 5-field methodology cap, that gates the CTA on the welcome screen. Started as an optional first-name field for headline personalization; became required once the result screen grew a real "email these results to Marcus" action, which needs to say who it's from. Still a single field that doesn't touch the math — see `_shared/brand-guidelines.md`'s Elements section for the full rationale. Marital status, dependents, and any other demographic fields were considered and rejected — they don't feed the 25x-rule math, and stacking them on top of the required 5 risks the brief's explicit "don't overbuild" instruction and judging criterion #3 (simple enough to finish).
