...
**Texture:** three small hand-drawn line doodles (a sunrise/horizon, an ascending arrow, a clock) sit fixed in the page margins, well clear of the card — the kind of stray sketches that end up in the corner of a real meeting whiteboard. Low opacity (~11%), ink-colored, never centered or competing with content. Hidden below `700px` viewport width, since on a phone the card fills nearly the whole screen and there's no real margin to put them in — mobile gets a flat `--board` with no texture at all rather than doodles crowding the form. (Superseded the dot-grid, and before that a card gloss gradient that read as a rendering glitch rather than a material cue — noted so a future pass doesn't retry either.)
