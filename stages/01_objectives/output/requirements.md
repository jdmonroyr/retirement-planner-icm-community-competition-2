...
One documented exception (workspace decision, not from the brief): an optional first-name field, outside the 5-field methodology cap, added post-launch for lead capture and headline personalization. It's a single skippable field that doesn't touch the math — see `_shared/brand-guidelines.md`'s Elements section for the rationale and the line it can't cross. Marital status, dependents, and any other demographic fields were considered and rejected — they don't feed the 25x-rule math, and stacking them on top of the required 5 risks the brief's explicit "don't overbuild" instruction and judging criterion #3 (simple enough to finish).
